...
def save_img_tensor_anim(img_tensor, path, markers=None, dpi=80, fps=30):
    ...
    line_ani = animation.FuncAnimation(
        fig, anim, all_steps, interval=1000 / fps, blit=False
    )
    line_ani.save(path)
    plt.close()


def save_depth_tensor_anim(
    depth_tensor, path, minv=None, maxv=None, markers=None, dpi=80, fps=30
):

    fig = plt.figure()
    f = plt.imshow(
        1 / depth_tensor[0, 0, ...].numpy(), cmap="magma", vmax=1 / minv, vmin=1 / maxv
    )
    s = plt.plot(markers[:, 0, 0], markers[:, 1, 0], "w+", markersize=5)
