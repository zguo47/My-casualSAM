...
class PointCloudVideoWebpage:
    ...
    def add_pointcloud(self, pts, rgb, mask):
        ...
        for i in range(pts.shape[0]):
            if mask[i] == 0:
                continue
            xyz_string.append(f"{pts[i, 0]:.4f},{pts[i, 1]:.4f},{pts[i, 2]:.4f},")
            rgb_string.append(f"{rgb[i, 0]:.4f},{rgb[i, 1]:.4f},{rgb[i, 2]:.4f},")
    ...
    def add_camera(self, R, t, rot_mat=None):
        ...
        for i in range(camera_xyz.shape[1]):
            camera_xyz_string += (
                f"{camera_xyz[0, i]:.4f},{camera_xyz[1, i]:.4f},{camera_xyz[2, i]:.4f},"
            )
